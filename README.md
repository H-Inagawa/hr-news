...
- **取得元**: Google News RSS (`hl=ja&gl=JP`) 
  - 以下の広範なキーワードを組み合わせて最新情報を取得します：
    `(労働基準法 OR 働き方改革 OR 社会保険 手続き OR 雇用契約 OR 労務管理) ニュース`
- **検証機能**: 要約時に「労務」「雇用」「働き方」などのキーワードが含まれているかをチェックし、関連性が低い場合は警告を表示します。
- **技術スタック**:
  - **Backend**: Node.js (Express), `rss-parser` (RSS解析), `axios` (Slack API通信), `dotenv`
  - **Frontend**: React (TypeScript), Vite, `axios` (API通信), Vanilla CSS
- **デザイン**: 
  - ダークモード対応のモダンなUI。
  - Glassmorphism（グラスモルフィズム）風のカードデザインと背景ぼかし。
  - レスポンシブ対応。
